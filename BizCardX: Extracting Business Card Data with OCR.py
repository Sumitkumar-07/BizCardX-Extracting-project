...
st.set_page_config(
    page_title="BizCardX: Extracting Business Card Data with OCR | By Sumit Kumar",
    page_icon=icon,
    layout="wide",
    initial_sidebar_state="expanded",
    menu_items={
        'About': """# This OCR app is created by *Sumit Kumar*!"""})
st.markdown(
    "<h1 style='text-align: center; color: #330000;'><u>BizCardX: Extracting Business Card Data with OCR</u></h1>",
...
# Create the "uploaded_cards" directory here
...
selected = option_menu(None, ["🏠Home", "Upload & Extract", "✒️Modify"],
                       icons=["home", "cloud-upload-alt", "edit"],
                       default_index=0,
                       orientation="horizontal",
                       styles={"nav-link": {"font-size": "25px",
                                            "text-align": "centre",
                                            "margin": "0px",
                                            "--hover-color": "#AB63FA",
...
mydb = sql.connect(host="your_host",
                   user="your_user",
                   password="your_pass",
...
if selected == "🏠Home":
    ...
    with col1:
        st.markdown(
            "## <font color='#4B0150'>🌐 Technologies Used :</font> <font color='#E1D9D1'>Python,easy OCR, Streamlit, SQL, Pandas.</font>",
            unsafe_allow_html=True
        )
        st.markdown(
            "## <font color='#4B0150'>📎Overview :</font> <font color='#E1D9D1'>In this streamlit web app you can upload an image of a business card and extract relevant information from it using easyOCR. You can view, modify or delete the extracted data in this app. This app would also allow users to save the extracted information into a database along with the uploaded business card image. The database would be able to store multiple entries, each with its own business card image and extracted information.</font>",
            unsafe_allow_html=True)
...
if selected == "Upload & Extract":
    st.markdown("### <font color='#4B0150'>Upload a Business Card</font>",
                unsafe_allow_html=True)

    uploaded_card = st.file_uploader("upload here",
    ...
    if uploaded_card is not None:
        ...
        def save_card(uploaded_card):
            with open(os.path.join("uploaded_cards", uploaded_card.name),
                      "wb") as f:
        ...
        with col1:
            ...
            st.markdown("#     ")
            st.markdown(
                "### <font color='#4B0150'>You have uploaded the card</font>",
                unsafe_allow_html=True)
        ...
        with col2:
            ...
            with st.spinner("Please wait processing image..."):
                ...
                st.markdown(
                    "### <font color='#4B0150'>Image Processed and Data Extracted</font>",
                    unsafe_allow_html=True)
...
if selected == "✒️Modify":
    ...
    col2.markdown(
        "<h2 style='color: #4B0150;'>Alter or Delete the data here</h2>",
        unsafe_allow_html=True)
    ...
    if st.button("View updated data"):
        ...
        updated_df = pd.DataFrame(mycursor.fetchall(),
                                  columns=["Company_Name", "Card_Holder",
                                           "Designation", "Mobile_Number",
                                           "Email", "Website", "Area", "City",
                                           "State", "Pin_Code"])
        st.write(updated_df)
